...
class Cube:
    ...
    def __init__(self, init_conf: CubeConf = None):
        if init_conf:
            self.cube = [
    ...
    def _apply_one_move(self, move: str) -> None:
        if move not in Cube.TRANSFORMATIONS:
            raise ValueError("Invalid move '%s'" % move)
        t = Cube.TRANSFORMATIONS[move]
        new_conf = [self.cube[t[i]] for i in range(0, Cube.NUM_FACELETS)]
        self.cube = new_conf

    def apply(self, move_sequence: str) -> None:
        moves = re.sub(r"([URFDLB])2", r"\1 \1", move_sequence).split()
        for move in moves:
